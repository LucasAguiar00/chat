	...
	class Connect{
		...
		function login($db, $username, $password){
		     try{
				...
				$stmt = $db->prepare('SELECT * FROM users WHERE  (user = :user or mail = :user) and password = md5(:password)');
			  	...
			  	if($stmt->rowCount()  > 0 ){
                    ...
                    if($user['status'] == "active"){
						...
						$_SESSION['user_ncc'] = $user['user'];

			$this->create($db, "all_logs", "DEFAULT, 'login','".$username."', '".date("Y-m-d")."','".date("H:i:s")."'"   ); 
